...
class GridManager:
    ...
    def build_grid(self, mid_price: float, levels: int, base_spacing_pct: float, volatility: float, regime: MarketRegime, order_size: float, regrid_threshold_pct: float, min_grid_profit_over_fees_pct: float, mode: GridMode) -> GridPlan:
        ...
        for i in range(1, levels + 1):
            if mode in (GridMode.NEUTRAL, GridMode.LONG_BIASED):
                long_levels.append(GridLevel(price=mid_price * (1 - spacing * i), side="buy", size=order_size))
            if mode in (GridMode.NEUTRAL, GridMode.SHORT_BIASED):
