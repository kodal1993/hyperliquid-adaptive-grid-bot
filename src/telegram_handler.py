...
class TelegramHandler:
    ...
    def format_status_report(self, report: dict) -> str:
        ...
        lines = [
            ...
            f"Flip trades today: {report.get('flip_trades_today', 0)}",
            f"Flip trade PnL: {money(report.get('flip_trade_pnl_today', report.get('flip_trade_pnl')))}",
            f"PnL by Regime: {report.get('pnl_by_regime', {})}",
            f"PnL by direction: {report.get('pnl_by_direction', {})}",
            f"PnL by close/flip: {report.get('pnl_by_trade_category', {})}",
            f"Avg winner / loser: {money(report.get('avg_winner'))} / {money(report.get('avg_loser'))}",
            f"Avg holding time: {num((report.get('average_holding_time_seconds') or 0) / 60, 1)} min",
            f"PnL long / short: {money(report.get('pnl_long_trades'))} / {money(report.get('pnl_short_trades'))}",
            "",
            "Utolsó 1 óra:",
