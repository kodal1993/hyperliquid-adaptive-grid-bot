...
class TelegramHandler:
    ...
    def format_status_report(self, report: dict) -> str:
        ...
        lines = [
            ...
            f"🎯 Grid Mode: {report.get('grid_mode', 'n/a')}",
            ...
            "",
            "📦 Position",
            f"Size: {num(report.get('position_size'))} {report.get('symbol', '')}",
            f"Notional: {money(report.get('position_notional'))}",
            f"Avg Entry: {money(report.get('avg_entry'))}",
            f"Mark Price: {money(report.get('mark_price'))}",
            "",
            "📊 Orders / Grid",
            ...
            f"Reason: {report.get('risk_reason', 'none')}",
            "",
            "⚡ Last Tick",
            f"Fills This Tick: {report.get('fills_count', 0)}",
            f"Last Fill: {report.get('last_fill', 'none')}",
            f"Updated: {ts}",
        ]
        return "\n".join(lines)[:3900]
