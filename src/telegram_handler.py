...
class TelegramHandler:
    ...
    def format_status_report(self, report: dict) -> str:
        ...
        lines = [
            ...
            "",
            "FILL RATE",
            f"1h fills: {(report.get('fill_rate_1h') or {}).get('orders_filled', 0)}",
            f"1h cancels: {(report.get('fill_rate_1h') or {}).get('orders_cancelled', 0)}",
            f"cancel/fill ratio: {num((report.get('fill_rate_1h') or {}).get('cancel_to_fill_ratio'), 2)}",
            f"nearest buy/sell distance: {pct(report.get('nearest_buy_distance_pct', report.get('distance_to_buy_pct')))} / {pct(report.get('nearest_sell_distance_pct', report.get('distance_to_sell_pct')))}",
            f"last fill age: {num(report.get('time_since_last_fill_minutes'), 1)} min",
            "",
            "EGÉSZSÉG (24h):",
            f"fills: {(report.get('trade_health_24h') or {}).get('fills_24h', 0)} | maker: {pct((report.get('trade_health_24h') or {}).get('maker_share_24h'))}",
            f"net PnL: {money((report.get('trade_health_24h') or {}).get('net_realized_24h'))} | fees: {money((report.get('trade_health_24h') or {}).get('fees_24h'))}",
            f"avg roundtrip net: {money((report.get('trade_health_24h') or {}).get('avg_roundtrip_net_24h'))}",
            f"ops/fill: {num((report.get('trade_health_24h') or {}).get('ops_per_fill_24h'), 1)} | ops/USDC: {num((report.get('trade_health_24h') or {}).get('ops_per_usdc_24h'), 2)}",
            f"request headroom: {num((report.get('rate_limit_budget') or {}).get('headroom'), 0)} | req/USDC össz: {num((report.get('rate_limit_budget') or {}).get('requests_per_usdc'), 2)}",
            "",
            "Market stress:",
            ...
            "Bias effect:",
            f"Long levels x{num(report.get('long_level_multiplier'), 2)}",
            f"Short levels x{num(report.get('short_level_multiplier'), 2)}",
            f"Long size x{num(report.get('long_size_multiplier'), 2)}",
            f"Short size x{num(report.get('short_size_multiplier'), 2)}",
            "",
            "Account:",
            ...
            f"OrderBook entries BULL/BEAR/FILTERED: {report.get('orderbook_bullish_entries', 0)} / {report.get('orderbook_bearish_entries', 0)} / {report.get('orderbook_filtered_entries', 0)}",
            f"Avg PnL OB bullish/bearish: {money(report.get('avg_pnl_bullish_entries'))} / {money(report.get('avg_pnl_bearish_entries'))}",
            f"Profit factor after fees: {num(report.get('profit_factor_after_fees'), 2)}",
            f"Net after fees: {money(report.get('net_pnl_after_fees', report.get('net_pnl')))}, W/L ratio: {num(report.get('win_loss_ratio'), 2)}",
            f"Avg expected/realized win/loss: {money(report.get('avg_expected_profit'))} / {money(report.get('avg_realized_profit'))} / {money(report.get('avg_realized_loss'))}",
            f"Fee leakage fills <10/<5: {report.get('fills_under_10_usd', 0)} / {report.get('fills_under_5_usd', 0)}, sources: {report.get('sub_10_fill_sources', {})}",
            f"Min notional blocked: {report.get('min_notional_blocked_count', 0)}, Dust cleanup: {report.get('dust_cleanup_count', 0)}, Anti-chop triggers: {report.get('anti_chop_trigger_count', 0)}",
            ...
            "Long vs Short:",
            f"LONG: Trades {report.get('long_trade_count', report.get('long_trades', 0))}, PnL {money(report.get('long_pnl', report.get('pnl_long_trades')))}, Winrate {num(report.get('long_winrate_pct'), 1)}%, PF {num(report.get('long_profit_factor'), 2)}",
            f"SHORT: Trades {report.get('short_trade_count', report.get('short_trades', 0))}, PnL {money(report.get('short_pnl', report.get('pnl_short_trades')))}, Winrate {num(report.get('short_winrate_pct'), 1)}%, PF {num(report.get('short_profit_factor'), 2)}",
