...
class RiskSnapshot:
    ...
    decision: str = "allow"
    global_margin_pct: float = 0.0
    global_leverage: float = 0.0
...
class RiskManager:
    ...
    def evaluate(
        ...
        max_position_notional: float = 1e18, 
        one_direction_exposure_pct: float = 0.0, 
        max_one_direction_exposure_pct: float = 1.0, 
        liquidation_distance_pct: float = 1.0, 
        min_liquidation_distance_pct: float = 0.05, 
        attempted_side: str = "none", 
        would_increase_exposure: bool = False,
        # Új, globális bálna-védelmi paraméterek
        global_margin_usage_pct: float = 0.0,
        max_global_margin_pct: float = 0.40,  # 40% fölött nincs új pozíció
        global_leverage: float = 0.0,
        max_global_leverage: float = 3.0      # Aggregált tőkeáttétel plafon
    ) -> RiskSnapshot:
        ...
        if blocked:
            ...
            # A likvidációs közelség minden esetben blokkol, kivéve a zárást
            if not exposure_reducing_override:
                blocked, reason = True, "liq_distance"
        elif would_increase_exposure:
            # Ezek a szűrők csak akkor futnak le, ha a bot új kitettséget (kockázatot) akar felvenni
            if global_margin_usage_pct > max_global_margin_pct:
                blocked, reason = True, "max_global_margin"
            elif global_leverage > max_global_leverage:
                blocked, reason = True, "max_global_leverage"
            elif position_notional > max_position_notional:
        ...
        return RiskSnapshot(
            ...
            exposure_reducing_override=exposure_reducing_override, 
            attempted_side=attempted_side, 
            decision=decision,
            global_margin_pct=global_margin_usage_pct,
