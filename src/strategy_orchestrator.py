...
class StrategyOrchestrator:
    ...
    def on_tick(self, candles: pd.DataFrame, equity: float, daily_pnl_pct: float, symbol: str, position_notional: float = 0.0) -> dict:
        regime = self.detector.detect(
            candles,
            trend_lookback_candles=self.config.trend_lookback_candles,
            trend_move_threshold_pct=self.config.trend_move_threshold_pct,
        ...
        if regime == MarketRegime.TREND_UP and mode == GridMode.LONG_BIASED:
            allow_sells = False
        if regime == MarketRegime.TREND_DOWN and mode == GridMode.SHORT_BIASED:
            allow_buys = False
